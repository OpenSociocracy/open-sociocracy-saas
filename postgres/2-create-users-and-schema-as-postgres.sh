...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER"  --dbname "$DB_NAME" <<-EOSQL
  BEGIN;
    CREATE USER $OPENSOCIOCRACY_USER WITH PASSWORD '$OPENSOCIOCRACY_PASS';
    GRANT CONNECT ON DATABASE $DB_NAME TO $OPENSOCIOCRACY_USER;
    CREATE SCHEMA IF NOT EXISTS opensociocracy_api;
    GRANT USAGE ON SCHEMA opensociocracy_api TO $OPENSOCIOCRACY_USER;
    GRANT ALL ON ALL SEQUENCES IN SCHEMA opensociocracy_api TO $OPENSOCIOCRACY_USER;
    GRANT ALL ON ALL TABLES IN SCHEMA opensociocracy_api TO $OPENSOCIOCRACY_USER;
    GRANT ALL ON ALL FUNCTIONS IN SCHEMA opensociocracy_api TO $OPENSOCIOCRACY_USER;
    ALTER ROLE $OPENSOCIOCRACY_USER SET search_path = opensociocracy_api,public;
